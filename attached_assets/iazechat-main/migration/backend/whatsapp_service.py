...
class WhatsAppService:
    ...
    def __init__(self, db):
        ...
        # Carregar configurações em runtime para garantir que .env foi carregado
        ...
        logger.info(f"🔧 WhatsAppService inicializado com Evolution API: {self.EVOLUTION_API_URL}")
    
    async def configure_webhook_for_instance(self, instance_name: str) -> bool:
        """Configurar webhook para uma instância existente - Evolution API v2.3.x"""
        try:
            ...
            logger.info(f"🔧 Configurando webhook para Evolution API v2.3.x: {instance_name}")
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code in [200, 201]:
                    logger.info(f"✅ Webhook configurado com sucesso (GitHub solution)!")
                    return True
                else:
    ...
    async def create_instance(self, reseller_id: str, instance_name: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"🌐 Evolution API URL configurada: {self.EVOLUTION_API_URL}")
            logger.info(f"🔑 Evolution API Key: {self.EVOLUTION_API_KEY[:20]}...")
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                # Criar payload SEM webhook para v2.2.3
                # Webhook será configurado após criação via endpoint separado
                create_payload = {
                ...
                logger.info(f"🆕 Criando instância {instance_name} (webhook será configurado depois)")
                ...
                if response.status_code in [200, 201]:
                    ...
                    # GARANTIR WEBHOOK: Configurar novamente após criação
                    logger.info(f"🔧 Garantindo configuração do webhook...")
                    ...
                    if webhook_configured:
                        logger.info(f"✅ Webhook GARANTIDO para instância {instance_name}")
                    else:
                        logger.warning(f"⚠️ Webhook pode não estar configurado - polling será usado como fallback")
