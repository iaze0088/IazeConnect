...
const WhatsAppQRCodeModal = ({ connection, onClose, onRefreshQR, onRestartSession }) => {
  ...
  const [qrCode, setQrCode] = useState(connection?.qr_code);
  ...
  useEffect(() => {
    if (step === 2 && connection?.status !== 'connected') {
      const checkInterval = setInterval(async () => {
        ...
        try {
          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/whatsapp/connections`, {
            headers: {
          ...
          if (response.ok) {
            ...
            const updatedConn = connections.find(c => c.id === connection.id);
