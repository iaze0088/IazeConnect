  ...
  useEffect(() => {
    const attemptAutoLogin = async () => {
      try {
        ...
        // Verificar se vem do /vendas
        const fromVendas = localStorage.getItem('iaze_from_vendas');
        const vendasWhatsapp = localStorage.getItem('iaze_whatsapp');
        const vendasPin = localStorage.getItem('iaze_pin');
        
        console.log('📦 [AUTO-LOGIN] localStorage completo:');
        console.log('  - iaze_from_vendas:', fromVendas);
        console.log('  - iaze_whatsapp:', vendasWhatsapp);
        console.log('  - iaze_pin:', vendasPin);
        
        let savedWhatsapp, savedPin;
        
        if (fromVendas === 'true' && vendasWhatsapp && vendasPin) {
          console.log('🎯 [AUTO-LOGIN] Detectado vindo do /vendas!');
          savedWhatsapp = vendasWhatsapp;
          savedPin = vendasPin;
          
          // Limpar flag para não repetir
          localStorage.removeItem('iaze_from_vendas');
          
          // Salvar no formato padrão
          const credentials = { whatsapp: savedWhatsapp, pin: savedPin };
          localStorage.setItem('client_credentials', JSON.stringify(credentials));
          console.log('💾 [AUTO-LOGIN] Credenciais salvas no formato padrão');
          ...
          savedWhatsapp = parsed.whatsapp;
          savedPin = parsed.pin;
        }
        
        if (!savedWhatsapp || !savedPin) {
          console.log('⚠️ [AUTO-LOGIN] Dados incompletos');
          setAutoLoggingIn(false);
          return;
        }
        ...
        if (error.response?.status !== 404 && error.response?.status !== 401) {
          toast.error('Erro ao fazer login automático. Por favor, tente novamente.');
        }
      }
    };
