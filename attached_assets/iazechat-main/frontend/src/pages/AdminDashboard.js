...
import { Shield, LogOut, Users, MessageSquare, Settings, Bell, Plus, Trash2, Edit, Bot, Folder, Zap, BookOpen, Save, Monitor, Share2, Copy, CheckCircle, Globe, ExternalLink, Phone, CreditCard, ChevronDown, Eye, Clock, Mail } from 'lucide-react';
...
import VendasBotManager from '../components/VendasBotManagerV2';
import VendasButtonsManager from '../components/VendasButtonsManager';
...
const AdminDashboard = () => {
  ...
  const [config, setConfig] = useState({ 
    ...
    allowed_data: { cpfs: [], emails: [], phones: [], random_keys: [], allowed_messages: [] },
  ...
  const [activeVendasTab, setActiveVendasTab] = useState('config');
  // State removido - Botões WA agora está dentro de WA Site
  ...
  const loadData = async () => {
    try {
      const [agentsRes, resellersRes, hierarchyRes, configRes, noticesRes, departmentsRes, iptvAppsRes] = await Promise.all([
        ...
        api.get('/config').catch(() => ({ data: { quick_blocks: [], auto_reply: [], apps: [], pix_key: '', allowed_data: { cpfs: [], emails: [], phones: [], random_keys: [], allowed_messages: [] }, api_integration: {}, ai_agent: {} } })),
      ...
      // Garantir que allowed_data sempre tenha allowed_messages
      const loadedConfig = configRes.data || {};
      if (!loadedConfig.allowed_data) {
        loadedConfig.allowed_data = { cpfs: [], emails: [], phones: [], random_keys: [], allowed_messages: [] };
      } else if (!loadedConfig.allowed_data.allowed_messages) {
        loadedConfig.allowed_data.allowed_messages = [];
      }
      
      // Garantir que quick_blocks sempre exista
      if (!loadedConfig.quick_blocks) {
        loadedConfig.quick_blocks = [];
      }
...
`.trim();
  ...
  const handleCreateNotice = async (title, message, recipientType = 'all', targetAudience = 'own', mediaUrl = '', mediaType = 'none') => {
    try {
      ...
      const mediaLabel = mediaType !== 'none' ? ` com ${mediaType === 'photo' ? '📷 foto' : mediaType === 'video' ? '🎥 vídeo' : '🎵 áudio'}` : '';
      toast.success(`✅ Aviso publicado para: ${recipientLabel}${mediaLabel}!`);
      loadData();
    } catch (error) {
    ...
    <div className="h-screen flex flex-col bg-slate-50">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          {/* Departments Tab (renomeado para I.A / Departamentos) */}
          ...
          <TabsContent value="security" className="space-y-6">
            <Card className="p-6">
              ...
              <div className="mb-6">
                <h4 className="font-semibold mb-3">💬 Mensagens Permitidas</h4>
                <p className="text-xs text-slate-500 mb-2">Mensagens que estarão salvas aqui poderão ser enviadas sem restrição de segurança</p>
                <div className="flex gap-2 mb-2">
                  <Input
                    placeholder="Digite a mensagem permitida..."
                    id="new-allowed-message"
                    onKeyPress={(e) => {
                      if (e.key === 'Enter') {
                        const input = document.getElementById('new-allowed-message');
                        const message = input.value.trim();
                        if (message) {
                          const messages = config.allowed_data?.allowed_messages || [];
                          setConfig({
                            ...config,
                            allowed_data: { ...config.allowed_data, allowed_messages: [...messages, message] }
                          });
                          input.value = '';
                        }
                      }
                ...
                <div className="flex flex-wrap gap-2">
                  {(config.allowed_data?.allowed_messages || []).map((message, idx) => (
                    <span key={idx} className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm flex items-center gap-2 max-w-md truncate">
                      {message}
                      <button onClick={() => {
                        const messages = config.allowed_data.allowed_messages.filter((_, i) => i !== idx);
                        setConfig({ ...config, allowed_data: { ...config.allowed_data, allowed_messages: messages } });
                        handleSaveConfig();
                      }} className="text-red-600 hover:text-red-800">×</button>
                    </span>
                  ))}
          ...
          <TabsContent value="vendas-bot" className="space-y-6">
            ...
            {/* Botões WA - Integrado dentro de WA Site */}
            <Card className="p-6 bg-gradient-to-r from-purple-50 to-blue-50 border-2 border-purple-300">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-purple-900">Botões Interativos WA</h3>
                  <p className="text-sm text-purple-700">Configure os botões do site de vendas</p>
                </div>
              </div>
              <VendasButtonsManager />
            </Card>
          </TabsContent>
          ...
          <TabsContent value="office" className="space-y-6">
            ...
            {/* API Integration - Movida para dentro da aba Office */}
      ...
      {/* Modal Botões Interativos REMOVIDO - Agora está dentro de WA Site */}
...
const NoticeForm = ({ onSubmit, isAdmin = false }) => {
  ...
  const [mediaType, setMediaType] = useState('none'); // 'none', 'photo', 'video', 'audio'
  const [mediaFile, setMediaFile] = useState(null);
  const [mediaUrl, setMediaUrl] = useState('');
  const [uploadingMedia, setUploadingMedia] = useState(false);

  const handleSubmit = () => {
    onSubmit(title, message, recipientType, targetAudience, mediaUrl, mediaType);
    setTitle('');
    setMessage('');
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Tipo de Mídia */}
      <div>
        <label className="block text-sm font-medium mb-2">📎 Anexar Mídia (Opcional)</label>
      ...
      {mediaType !== 'none' && (
        <div className="space-y-2 p-4 bg-blue-50 border border-blue-200 rounded">
          <label className="text-sm font-medium text-blue-800">
            {mediaType === 'photo' && '📷 Carregar Foto'}
            {mediaType === 'video' && '🎥 Carregar Vídeo'}
            {mediaType === 'audio' && '🎵 Carregar Áudio'}
          </label>
          <Input
            type="file"
            accept={
          ...
          {mediaUrl && !uploadingMedia && (
            <p className="text-sm text-green-600">✅ Mídia carregada com sucesso!</p>
          )}
        </div>
      )}

      <Button 
        data-testid="publish-notice-btn" 
        onClick={handleSubmit} 
        className="w-full bg-purple-600 hover:bg-purple-700"
        disabled={!title || !message || uploadingMedia}
      >
