...
import { Headphones, LogOut, Send, Paperclip, Mic, Phone, User, Key, RefreshCw, Bot, BookOpen, Search, X, Monitor, Copy, ExternalLink } from 'lucide-react';
...
const AgentDashboard = () => {
  ...
  useEffect(() => {
    ...
    loadTutorials();
    loadIPTVApps();
    calculateFilteredCounts(); // Calcular counts filtrados
    const interval = setInterval(() => {
      loadCounts();
      calculateFilteredCounts(); // Atualizar counts filtrados
    }, 5000);
    return () => clearInterval(interval);
  }, [status, originFilter]); // Recarregar quando status OU originFilter mudar
  ...
  const loadTickets = async (forceReload = false) => {
    try {
      ...
      // Chave do cache
      const cacheKey = `${backendStatus}_${originFilter}`;
      
      // Se tem cache e não é reload forçado, usar cache (INSTANTÂNEO)
      if (!forceReload && ticketsCache[cacheKey]) {
        console.log('⚡ Usando cache (instantâneo):', cacheKey);
        setTickets(ticketsCache[cacheKey]);
        return;
      }
      
      setIsLoadingTickets(true); // Mostrar loading
      console.log('🔄 Carregando do backend...', { status: backendStatus, origin: originFilter });
      
      const startTime = performance.now();
      
      // **OTIMIZAÇÃO: Passar origin como parâmetro + LIMIT**
      const { data } = await api.get('/tickets', { 
        params: { 
          status: backendStatus,
          origin: originFilter,
          limit: 50  // ✅ LIMITAR A 50 TICKETS POR ABA
        } 
      });
      
      const endTime = performance.now();
      console.log(`📥 ${data.length} tickets em ${(endTime - startTime).toFixed(0)}ms`);
  ...
  const handleSyncTickets = async () => {
    setIsSyncing(true);
    toast.info('🔄 Sincronizando tickets...');
    ...
    try {
      ...
      await calculateFilteredCounts();
      
      toast.success('✅ Tickets sincronizados!');
    } catch (error) {
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex gap-1 p-2 bg-slate-50 border-b flex-shrink-0">
                ...
                <Button
                  ...
                  disabled={isSyncing}
                  className="px-3 bg-white hover:bg-blue-50 text-blue-600 border-blue-300"
