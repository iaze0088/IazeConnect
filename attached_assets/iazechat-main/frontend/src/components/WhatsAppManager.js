...
const WhatsAppManager = () => {
  ...
  const handleSyncAllStatus = async () => {
    try {
      ...
      const response = await api.post('/whatsapp/sync-all-status');
      console.log('✅ Sincronização concluída:', response.data);
      
      // Recarregar dados após sincronizar
      await loadData();
      
      const { synced, updated_connections } = response.data;
      const connected = updated_connections.filter(c => c.connected).length;
      
      if (connected > 0) {
        alert(`✅ Sincronização concluída!\n${connected} conexão(ões) ativa(s) detectada(s).`);
      } else {
        alert(`ℹ️ Sincronização concluída.\n${synced} conexão(ões) verificada(s), nenhuma ativa no momento.`);
      }
      
    } catch (error) {
      console.error('❌ Erro ao sincronizar status:', error);
      alert('Erro ao sincronizar status das conexões.');
    }
  };
  ...
  const handleAddConnection = async () => {
    ...
    console.log('🔧 [DEBUG] handleAddConnection iniciado - Versão 2.0.4');
    
    try {
      const name = prompt('Nome da conexão (ex: Atendimento 1):', 'WhatsApp Principal');
      if (!name) return;
      ...
      try {
        ...
        console.log('🔍 DEBUG - qr_code:', response.data.qr_code ? 'SIM' : 'NÃO');
        console.log('🔍 DEBUG - qr_code_base64:', response.data.qr_code_base64 ? 'SIM' : 'NÃO');
        
        // Adicionar a nova conexão no array antes de abrir o modal
        setConnections(prev => [...prev, response.data]);
        
        // **SEMPRE ABRIR MODAL - buscar QR code se necessário**
        console.log('🎉 Abrindo modal automaticamente...');
        console.log('🔍 selectedConnection ID:', response.data.id);
        setSelectedConnection(response.data.id);  // Salvar apenas o ID
        setSelectedConnectionData(response.data);  // Salvar dados completos da conexão
        setShowQRModal(true);
        ...
        if (response.data.qr_code || response.data.qr_code_base64) {
          ...
          setTimeout(async () => {
            try {
              const qrResponse = await api.get(`/whatsapp/connections/${response.data.id}/qr`);
              console.log('📲 QR Code obtido:', qrResponse.data);
              // Atualizar a conexão no array com o QR code
              const updatedConnection = { ...response.data, qr_code: qrResponse.data.qr_code, qr_code_base64: qrResponse.data.qr_code_base64 };
              setConnections(prev => prev.map(c => 
                c.id === response.data.id 
                  ? updatedConnection
                  : c
              ));
              // Atualizar também o selectedConnectionData
              setSelectedConnectionData(updatedConnection);
            } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        ...
        >
          ...
          Sincronizar Status de Todas Conexões
        </Button>
      </div>
      ...
      {/* Novo Modal QR Code */}
      {showQRModal && selectedConnectionData && (
        <WhatsAppQRCodeModal
