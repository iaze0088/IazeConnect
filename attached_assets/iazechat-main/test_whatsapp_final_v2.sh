...
echo "⏳ Aguardando até 70s para QR code ser gerado..."
CREATE_RESPONSE=$(curl -s -X POST "$BACKEND_URL/whatsapp/connections" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Teste Final '$(date +%H%M%S)'"}' \
  --max-time 75)

CONNECTION_ID=$(echo "$CREATE_RESPONSE" | jq -r '.connection.id // .id')
QR_CODE=$(echo "$CREATE_RESPONSE" | jq -r '.connection.qr_code // .qr_code')
