#!/usr/bin/env python3
"""
🧪 TESTE SISTEMÁTICO - ABA 7: DADOS PERMITIDOS (V2)
Testa funcionalidades de gerenciamento de dados permitidos: PIX, CPFs, Emails, Telefones, Chaves Aleatórias
Versão corrigida que preserva dados existentes ao fazer updates
"""
...
class ABA7DadosPermitidosTestV2:
    def __init__(self):
        self.session = None
        ...
        try:
            ...
            async with self.session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    self.current_config = config  # Salvar config atual
    ...
    async def update_config_preserving_data(self, updates: dict):
        """Helper para atualizar config preservando dados existentes"""
        if not self.current_config:
            raise Exception("Config atual não carregada")
            
        # Criar payload completo baseado na config atual
        payload = {
            "quick_blocks": self.current_config.get("quick_blocks", []),
            "auto_reply": self.current_config.get("auto_reply", []),
            "apps": self.current_config.get("apps", []),
            "support_avatar": self.current_config.get("support_avatar", ""),
            "pix_key": self.current_config.get("pix_key", ""),
            "manual_away_mode": self.current_config.get("manual_away_mode", False),
            "away_message": self.current_config.get("away_message", "Estamos ausentes no momento. Por favor, aguarde que retornaremos em breve."),
            "allowed_data": self.current_config.get("allowed_data", {"cpfs": [], "emails": [], "phones": [], "random_keys": [], "allowed_messages": []}),
            "api_integration": self.current_config.get("api_integration", {"api_url": "", "api_token": "", "api_enabled": False}),
            "ai_agent": self.current_config.get("ai_agent", {
                "name": "Assistente IA",
                "personality": "",
                "instructions": "",
                "llm_provider": "openai",
            ...
            "ai_globally_enabled": self.current_config.get("ai_globally_enabled", True)
        }
        
        # Aplicar updates
        for key, value in updates.items():
            if key == "allowed_data":
                # Merge allowed_data preservando campos existentes
                current_allowed = payload["allowed_data"]
                for sub_key, sub_value in value.items():
                    current_allowed[sub_key] = sub_value
            else:
                payload[key] = value
    ...
    async def test_3_save_pix_key(self):
        ...
        try:
            ...
            # Criar payload completo preservando dados existentes
            payload = await self.update_config_preserving_data({
                "pix_key": "chave_pix_teste@example.com"
            })
            ...
            async with self.session.put(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    self.current_config["pix_key"] = "chave_pix_teste@example.com"
                    return True
                else:
                    error_text = await response.text()
        ...
        try:
            ...
            async with self.session.put(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    # Atualizar config atual
                    self.current_config["allowed_data"]["cpfs"] = ["123.456.789-00", "987.654.321-00"]
                    return True
                else:
                    error_text = await response.text()
        ...
        try:
            ...
            async with self.session.put(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    # Atualizar config atual
                    self.current_config["allowed_data"]["emails"] = ["teste@example.com", "admin@example.com"]
                    return True
                else:
                    error_text = await response.text()
        ...
        try:
            ...
            async with self.session.put(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    # Atualizar config atual
                    self.current_config["allowed_data"]["phones"] = ["+5511999999999", "+5521988888888"]
                    return True
                else:
                    error_text = await response.text()
        ...
        try:
            ...
            async with self.session.put(url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    # Atualizar config atual
                    self.current_config["allowed_data"]["random_keys"] = ["chave-random-123", "chave-random-456"]
                    return True
                else:
                    error_text = await response.text()
    ...
    async def run_all_tests(self):
        ...
        print("🧪 INICIANDO TESTE SISTEMÁTICO - ABA 7: DADOS PERMITIDOS (V2)")
        print("=" * 80)
        print(f"🎯 Backend URL: {BACKEND_URL}")
        print(f"👤 Admin: {ADMIN_EMAIL}")
        print("🔧 Versão corrigida que preserva dados existentes")
        print("=" * 80)
    ...
    def print_final_report(self):
        ...
        print("\n" + "=" * 80)
        print("📊 RELATÓRIO FINAL - ABA 7: DADOS PERMITIDOS (V2)")
        print("=" * 80)
...
async def main():
    """Função principal"""
    test = ABA7DadosPermitidosTestV2()
