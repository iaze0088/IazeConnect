...
async def create_ai_agent(
    ...
    agent = {
        ...
        "knowledge_restriction": False,
        "timezone": "America/Sao_Paulo",
        "is_active": data.is_active if hasattr(data, 'is_active') else False,  # ✅ Usar valor do data ou False
        "schedule_start_time": data.schedule_start_time if hasattr(data, 'schedule_start_time') else None,  # ✅ Novo campo
        "schedule_end_time": data.schedule_end_time if hasattr(data, 'schedule_end_time') else None,  # ✅ Novo campo
    ...
    print(f"🔍 DEBUG: Criando agent com is_active={agent['is_active']}, schedule_start_time={agent['schedule_start_time']}, schedule_end_time={agent['schedule_end_time']}")
...
async def update_department(
    ...
    # Atualizar apenas campos fornecidos (ACEITAR null para desvincular IA)
    # IMPORTANTE: Não usar exclude_unset para permitir ai_agent_id=null
    update_data = {}
    for field, value in data.dict().items():
        if field == 'ai_agent_id':
            # SEMPRE incluir ai_agent_id, mesmo se for null (desvincular)
            update_data[field] = value
        elif value is not None:
            # Outros campos: apenas se não for None
            update_data[field] = value
