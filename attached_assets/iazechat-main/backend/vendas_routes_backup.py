...
async def start_vendas_session(
    ...
    try:
        ...
        logger.info(f"🔘 Button config loaded - enabled: {button_config.is_enabled}, mode: {button_config.mode}, buttons: {len(button_config.root_buttons)}")
        ...
        # 🆕 RETORNAR BOTÕES SE CONFIGURADO
        response_data = {
            "session_id": session_id,
            "messages": [{
                "message_id": welcome_message_id,
        ...
        # Se sistema de botões está ativo, retornar botões
        if button_config.is_enabled and button_config.mode in ["button", "hibrido"]:
            logger.info(f"🔘 Sistema de botões ativo - Modo: {button_config.mode}")
            response_data["button_config"] = {
                "is_enabled": True,
            ...
            logger.info("ℹ️ Sistema de botões desabilitado ou modo IA")
            response_data["button_config"] = {
                "is_enabled": False,
                "mode": "ia"
            }
...
async def send_vendas_message(
    ...
    try:
        ...
        # 🆕 VERIFICAR SISTEMA DE BOTÕES ANTES DE PROCESSAR
        buttons_service = ButtonsService(db)
        button_config = await buttons_service.get_config()
        
        print(f"🔘🔘🔘 Button config - Enabled: {button_config.is_enabled}, Mode: {button_config.mode}")
        logger.info(f"🔘 Button config - Enabled: {button_config.is_enabled}, Mode: {button_config.mode}, Type enabled: {type(button_config.is_enabled)}, Type mode: {type(button_config.mode)}")
        ...
        if button_config.is_enabled and button_config.mode == "button":
            ...
            # Retornar botões atuais
            current_buttons = await buttons_service.get_buttons_for_user(request.session_id)
            ...
            return {
                ...
                "bot_state": "active",
                "buttons": [b.dict() for b in current_buttons]
            }
        ...
        # 🆕 VERIFICAR SE IA ESTÁ PERMITIDA (modo "ia" ou "hibrido", NÃO "button")
        ia_permitida = (
            button_config.mode in ["ia", "hibrido"] and 
            usa_ia and 
        ...
        if ia_permitida:
            logger.info(f"🚀 Usando IA HUMANIZADA - Modo: {button_config.mode}")
...
async def handle_button_click(
    ...
    try:
        ...
        # Salvar mensagem do bot com resposta do botão
        bot_message_id = str(uuid.uuid4())
        bot_message = {
            "message_id": bot_message_id,
            "session_id": session_id,
